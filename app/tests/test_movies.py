...
class TestMoviesJob:
    ...
    def test_run_job(self, spark_session, mocker):
        test_config = {"output_data_path": "data/test_data_output"}
        shutil.rmtree(f"../{test_config.get('output_data_path')}")
        test_data = spark_session.createDataFrame(
            [(1, "Toy Story (1995)", "Adventure"), (160646, "Goat (2016)", "Drama")],
            ["movieId", "title", "genres"],
        )
        ...
        assert os.path.exists(f"../{test_config.get('output_data_path')}")